...
export const messageStorage: MessageStorage = (() => {
  // Electron 环境：使用 Electron API（如果有的话）
  if (isElectron) {
    const electronAPI = (window as any).electronAPI
    // 检查 Electron API 是否有消息存储方法
    if (electronAPI?.getMessages && electronAPI?.saveMessages) {
      return {
        ...
        saveMessages: async (messages) => {
          try {
            await electronAPI.saveMessages(messages)
            return { success: true }
          } catch (e: any) {
    ...
    clearMessages: async () => {
      try {
        if (isCapacitor) {
          try {
            const { Preferences } = await import('@capacitor/preferences')
            await Preferences.remove({ key: MESSAGE_STORAGE_KEY })
            return { success: true }
          } catch {
